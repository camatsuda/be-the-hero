...
IMPORTANT: Update the import statement below to match your actual module path.
Replace 'notebooks.dataproducts.webapp.ingestion' with your actual import path.

Run tests with: pytest test_ingestion.py -v --cov=ingestion --cov-report=xml
"""
...
# ============================================================================
# IMPORTANT: Update this import to match your actual module structure
# ============================================================================
...
# Module name for patching - UPDATE THIS to match your module path
MODULE_NAME = 'notebooks.dataproducts.webapp.ingestion'
...
class TestWarehouseManager:
    ...
    @patch(f'{MODULE_NAME}.requests.get')
...
class TestDataLoader:
    ...
    @patch(f'{MODULE_NAME}.sql.connect')
...
class TestETLPipeline:
    ...
    def test_pipeline_initialization(self, mock_dbutils_global, mock_spark, pipeline_config):
        ...
        with patch(f'{MODULE_NAME}.WarehouseManager'):
            with patch(f'{MODULE_NAME}.DataLoader'):
    ...
    def test_run_with_default_date(self, mock_dbutils_global, mock_spark, pipeline_config):
        ...
        with patch(f'{MODULE_NAME}.WarehouseManager'):
            with patch(f'{MODULE_NAME}.DataLoader') as mock_loader_class:
                mock_loader = Mock()
...
class TestMainEntryPoint:
    ...
    @patch(f'{MODULE_NAME}.ETLPipeline')
...
class TestEndToEndIntegration:
    ...
    def test_full_pipeline_happy_path(self, mock_dbutils_global, mock_spark, pipeline_config):
        ...
        with patch(f'{MODULE_NAME}.WarehouseManager') as mock_wh_class:
            with patch(f'{MODULE_NAME}.DataLoader') as mock_loader_class:
                with patch(f'{MODULE_NAME}.DeltaWriter') as mock_writer_class:
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--tb=short", "--cov=notebooks.dataproducts.webapp.ingestion", "--cov-report=xml"])
